...
const getAllWineTypes = async (req, res) => {
  try {
    managerService.getAllWineTypes((err, data) => {
      if (err) return res.status(500).json({ error: 'שגיאה פנימית' });
      res.json(data);
    });
  } catch (err) {
...
const updateWineType = (req, res) => {
  const  WineTypeID = req.body.WineTypeID;
  managerService.updateWineType(WineTypeID,req.body, (err, data) => {
    if (err) return res.status(400).json({ error: err.message });
    res.json({ success: true, data });
  });
};

const deleteWineType = (req, res) => {
  const { WineTypeID } = req.body;
  managerService.deleteWineType(WineTypeID, (err) => {
    if (err) {
      if (err.code === 'RELATED_PRODUCTS_EXIST') {
        return res.status(200).json({ success: true, message: err.message });
      }
    return res.status(500).json({ error: err });}
    res.status(201).json({ success: true, message: 'סוג יין נמחק' });
  });
};

const getAllTourTypes = async (req, res) => {
  try {
    managerService.getAllTourTypes((err, data) => {
      if (err) return res.status(500).json({ error: 'שגיאה פנימית' });
      res.json(data);
    });
  } catch (err) {
...
const updateTourType = (req, res) => {
  const TourTypeID= req.body.TourTypeID;
  managerService.updateTourType(TourTypeID,req.body, (err, data) => {
    if (err) return res.status(400).json({ error: err.message });
    res.json({ success: true, data });
  });
};

const deleteTourType = (req, res) => {
  const { TourTypeID } = req.body;
  managerService.deleteTourType(TourTypeID, (err) => {
    if (err) 
    {
      if (err.code === 'RELATED_PRODUCTS_EXIST') {
        return res.status(200).json({ success: true, message: err.message });
      ...
      return res.status(500).json({ error: err });}
    res.status(201).json({ success: true, message: 'סוג סיור נמחק' });
  });
};
